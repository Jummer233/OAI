                                 ...
                                 NR_DL_FRAME_PARMS *frame_parms)
{
  ...
  int start_re = (frame_parms->first_carrier_offset + (pusch_pdu->rb_start + pusch_pdu->bwp_start) * NR_NB_SC_PER_RB)
                                   ...
                                   int shift_ch_ext)
{
  ...
  LOG_D(PHY,
        "Scaling PUSCH Chest in OFDM symbol %d by %d, pilots %d nb_rb %d NCP %d symbol %d\n",
        symbol,
        ch_amp,
        is_dmrs_symbol,
        nb_rb,
  ...
  for (int aatx = 0; aatx < nrOfLayers; aatx++) {
    for (int aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
      ...
      avg[aatx * frame_parms->nb_antennas_rx + aarx] =
          (((int32_t *)&avg128U)[0] + ((int32_t *)&avg128U)[1] + ((int32_t *)&avg128U)[2] + ((int32_t *)&avg128U)[3]) / y;
    }
  }
                                          ...
                                          uint32_t output_shift)
{
  ...
  simde__m256i complex_shuffle256 = simde_mm256_set_epi8(29,
                                                         28,
                                                         31,
                                                         30,
                                                         25,
                             ...
                             int32_t shift)
{
  ...
  for (rb = 0; rb < 3 * nb_rb; rb++) {
    ...
    // ad_im_128 = simde_mm_shufflelo_epi16(after_mf_00_128[0], SIMDE_MM_SHUFFLE(2,3,0,1));//permutes IQs for the low 64 bits as
    // [I_a0 Q_a1 I_a2 Q_a3]_64bits to [Q_a1 I_a0 Q_a3 I_a2]_64bits ad_im_128 = simde_mm_shufflehi_epi16(ad_im_128,
    // SIMDE_MM_SHUFFLE(2,3,0,1));//permutes IQs for the high 64 bits as [I_a0 Q_a1 I_a2 Q_a3]_64bits to [Q_a1 I_a0 Q_a3
    // I_a2]_64bits ad_im_128 = simde_mm_madd_epi16(ad_im_128,after_mf_11_128[0]);//Im: (Q_aI_d + I_aQ_d)
    ...
    // bc_im_128 = simde_mm_shufflelo_epi16(after_mf_01_128[0], SIMDE_MM_SHUFFLE(2,3,0,1));//permutes IQs for the low 64 bits as
    // [I_b0 Q_b1 I_b2 Q_b3]_64bits to [Q_b1 I_b0 Q_b3 I_b2]_64bits bc_im_128 = simde_mm_shufflehi_epi16(bc_im_128,
    // SIMDE_MM_SHUFFLE(2,3,0,1));//permutes IQs for the high 64 bits as [I_b0 Q_b1 I_b2 Q_b3]_64bits to [Q_b1 I_b0 Q_b3
    // I_b2]_64bits bc_im_128 = simde_mm_madd_epi16(bc_im_128,after_mf_10_128[0]);//Im: (Q_bI_c + I_bQ_c)
 ...
 *
 * */
static void nr_ulsch_conjch0_mult_ch1(int *ch0, int *ch1, int32_t *ch0conj_ch1, unsigned short nb_rb, unsigned char output_shift0)
{
                                           ...
                                           simde__m128i det)
{
  ...
  // the imag part
  xy_im_128 = simde_mm_shufflelo_epi16(input_x, SIMDE_MM_SHUFFLE(2, 3, 0, 1)); // permutes IQs for the low 64 bits as [I_a0 Q_a1
                                                                               // I_a2 Q_a3]_64bits to [Q_a1 I_a0 Q_a3 I_a2]_64bits
  xy_im_128 = simde_mm_shufflehi_epi16(
      xy_im_128,
      SIMDE_MM_SHUFFLE(2,
                       3,
                       0,
                       1)); // permutes IQs for the high 64 bits as [I_a0 Q_a1 I_a2 Q_a3]_64bits to [Q_a1 I_a0 Q_a3 I_a2]_64bits
  ...
  // the imag part
  wz_im_128 = simde_mm_shufflelo_epi16(input_w, SIMDE_MM_SHUFFLE(2, 3, 0, 1)); // permutes IQs for the low 64 bits as [I_a0 Q_a1
                                                                               // I_a2 Q_a3]_64bits to [Q_a1 I_a0 Q_a3 I_a2]_64bits
  wz_im_128 = simde_mm_shufflehi_epi16(
      wz_im_128,
      SIMDE_MM_SHUFFLE(2,
                       3,
                       0,
                                     ...
                                     uint32_t buffer_length)
{
  ...
  if (n_rx >= 2) {
    ...
    nr_ulsch_conjch0_mult_ch1(ch00, ch00, conjch00_ch00, nb_rb_0, shift);
    // (1/2^log2_maxh)*conj_H_10xH_10: (1/(64*2))conjH_10*H_10*2^15
    nr_ulsch_conjch0_mult_ch1(ch10, ch10, conjch10_ch10, nb_rb_0, shift);
    // conj_H_00xH_01
    nr_ulsch_conjch0_mult_ch1(ch00, ch01, conjch00_ch01, nb_rb_0,
                              shift); // this shift is equal to the channel level log2_maxh
    // conj_H_10xH_11
    nr_ulsch_conjch0_mult_ch1(ch10, ch11, conjch10_ch11, nb_rb_0, shift);
    // conj_H_01xH_01
    nr_ulsch_conjch0_mult_ch1(ch01, ch01, conjch01_ch01, nb_rb_0, shift);
    // conj_H_11xH_11
    nr_ulsch_conjch0_mult_ch1(ch11, ch11, conjch11_ch11, nb_rb_0, shift);
    // conj_H_01xH_00
    nr_ulsch_conjch0_mult_ch1(ch01, ch00, conjch01_ch00, nb_rb_0, shift);
    // conj_H_11xH_10
    nr_ulsch_conjch0_mult_ch1(ch11, ch10, conjch11_ch10, nb_rb_0, shift);
  }
  if (n_rx == 4) {
    // (1/2^log2_maxh)*conj_H_20xH_20: (1/(64*2*16))conjH_20*H_20*2^15
    nr_ulsch_conjch0_mult_ch1(ch20, ch20, conjch20_ch20, nb_rb_0, shift);

    // (1/2^log2_maxh)*conj_H_30xH_30: (1/(64*2*4))conjH_30*H_30*2^15
    nr_ulsch_conjch0_mult_ch1(ch30, ch30, conjch30_ch30, nb_rb_0, shift);

    // (1/2^log2_maxh)*conj_H_20xH_20: (1/(64*2))conjH_20*H_20*2^15
    nr_ulsch_conjch0_mult_ch1(ch20, ch21, conjch20_ch21, nb_rb_0, shift);

    nr_ulsch_conjch0_mult_ch1(ch30, ch31, conjch30_ch31, nb_rb_0, shift);

    nr_ulsch_conjch0_mult_ch1(ch21, ch21, conjch21_ch21, nb_rb_0, shift);

    nr_ulsch_conjch0_mult_ch1(ch31, ch31, conjch31_ch31, nb_rb_0, shift);

    // (1/2^log2_maxh)*conj_H_20xH_20: (1/(64*2))conjH_20*H_20*2^15
    nr_ulsch_conjch0_mult_ch1(ch21, ch20, conjch21_ch20, nb_rb_0, shift);

    nr_ulsch_conjch0_mult_ch1(ch31, ch30, conjch31_ch30, nb_rb_0, shift);
  ...
  for (int rb = 0; rb < 3 * nb_rb_0; rb++) {
    ...
    // printf("layer_1 \n");
    mmtmpD0 =
        nr_ulsch_comp_muli_sum(rxdataF_comp128_0[0], after_mf_d_128[0], rxdataF_comp128_1[0], after_mf_b_128[0], determ_fin_128[0]);

    // printf("layer_2 \n");
    mmtmpD1 =
        nr_ulsch_comp_muli_sum(rxdataF_comp128_1[0], after_mf_a_128[0], rxdataF_comp128_0[0], after_mf_c_128[0], determ_fin_128[0]);
                     ...
                     uint32_t nvar)
{
  ...
  if (rel15_ul->pdu_bit_map & PUSCH_PDU_BITMAP_PUSCH_PTRS) {
    nr_pusch_ptrs_processing(gNB, frame_parms, rel15_ul, ulsch_id, slot, symbol, buffer_length);
        ...
        for (int l = 0; l < rel15_ul->nrOfLayers; l++)
          for (int m = 0; m < rel15_ul->qam_mod_order; m++)
            llr_ptr[i * rel15_ul->nrOfLayers * rel15_ul->qam_mod_order + l * rel15_ul->qam_mod_order + m] =
                rdata->llr_layers[l][pusch_vars->llr_offset[symbol] + i * rel15_ul->qam_mod_order + m];
    }
    // unscrambling
...
int nr_rx_pusch_tp(PHY_VARS_gNB *gNB, uint8_t ulsch_id, uint32_t frame, uint8_t slot, unsigned char harq_pid)
{
  ...
  for (meas_symbol = rel15_ul->start_symbol_index; meas_symbol < (rel15_ul->start_symbol_index + rel15_ul->nr_of_symbols);
       meas_symbol++)
  ...
  for (uint8_t symbol = rel15_ul->start_symbol_index; symbol < (rel15_ul->start_symbol_index + rel15_ul->nr_of_symbols);
    ...
    for (int s = 0; s < numSymbols; s++) {
      ...
      pusch_vars->llr_offset[symbol + s] =
          ((symbol + s) == rel15_ul->start_symbol_index)
              ? 0
    ...
    if (total_res > 0) {
      ...
      id.p = 1 + symbol;
      notifiedFIFO_elt_t *req = newNotifiedFIFO_elt_with_taskid(sizeof(puschSymbolProc_t),
                                                                id.p,
                                                                &gNB->respPuschSymb,
                                                                &nr_pusch_symbol_processing,
                                                                ulsch_demodulation_task_id++); // create a job for Tpool
