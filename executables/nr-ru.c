...
void fh_if5_south_in(RU_t *ru, int *frame, int *tti)
{
  ...
  if (proc->first_rx == 0) {
    if (proc->tti_rx != *tti) {
      LOG_E(PHY,
            "Received Timestamp doesn't correspond to the time we think it is (proc->tti_rx %d, subframe %d)\n",
            proc->tti_rx,
            *tti);
      if (!oai_exit)
        exit_fun("Exiting");
    ...
    if (proc->frame_rx != *frame) {
      LOG_E(
          PHY,
          "Received Timestamp doesn't correspond to the time we think it is (proc->frame_rx %d frame %d proc->tti_rx %d tti %d)\n",
          proc->frame_rx,
          *frame,
          proc->tti_rx,
          *tti);
  ...
  if (fhtime > 800)
    LOG_W(PHY,
          "IF5 %d.%d => RX %d.%d first_rx %d: time %f, rxstart %d\n",
          *frame,
          *tti,
          proc->frame_rx,
          proc->tti_rx,
...
void fh_if4p5_south_in(RU_t *ru, int *frame, int *slot)
{
  ...
  do { // Blocking, we need a timeout on this !!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (packet_type == IF4p5_PULFFT)
      proc->symbol_mask[sl] = proc->symbol_mask[sl] | (1 << symbol_number);
  ...
  if (proc->first_rx == 0) {
    if (proc->tti_rx != *slot) {
      LOG_E(PHY,
            "Received Timestamp (IF4p5) doesn't correspond to the time we think it is (proc->tti_rx %d, subframe %d)\n",
            proc->tti_rx,
            *slot);
      exit_fun("Exiting");
    }
...
void fh_if5_north_asynch_in(RU_t *ru, int *frame, int *slot)
{
  ...
  tti_tx = (idx_sf * fp->slots_per_subframe + (int)round((float)(timestamp_tx % fp->samples_per_subframe) / fp->samples_per_slot0))
           % (fp->slots_per_frame);
  ...
  if (proc->first_tx != 0) {
    ...
    *frame = frame_tx;
    ...
    AssertFatal(tti_tx == *slot, "tti_tx %d is not what we expect %d\n", tti_tx, *slot);
...
void fh_if4p5_north_asynch_in(RU_t *ru, int *frame, int *slot)
{
  ...
  do {
    ...
    LOG_D(PHY,
          ...
          *slot,
          nr_slot_select(cfg, frame_tx, *slot),
          frame_tx,
          slot_tx,
          symbol_number);
    ...
    if (proc->first_tx != 0) {
      ...
      *slot = slot_tx;
      ...
      AssertFatal(frame_tx == *frame, "frame_tx %d is not what we expect %d\n", frame_tx, *frame);
      AssertFatal(slot_tx == *slot, "slot_tx %d is not what we expect %d\n", slot_tx, *slot);
    }
  ...
  proc->timestamp_tx = (((uint64_t)frame_tx + (uint64_t)proc->frame_tx_unwrap) * fp->samples_per_subframe * 10)
                       + fp->get_samples_slot_timestamp(slot_tx, fp, 0);
  LOG_D(PHY,
        "RU %d/%d TST %llu, frame %d, subframe %d\n",
        ru->idx,
        0,
        (long long unsigned int)proc->timestamp_tx,
        frame_tx,
        slot_tx);
...
static void rx_rf(RU_t *ru, int *frame, int *slot)
{
  ...
  if (proc->first_rx == 0) {
    if (proc->tti_rx != *slot) {
      LOG_E(PHY,
            "Received Timestamp (%llu) doesn't correspond to the time we think it is (proc->tti_rx %d, slot %d)\n",
    ...
    if (slot_type == NR_MIXED_SLOT) {
      ...
      if (fp->slots_per_subframe == 1) {
        if (txsymb <= 7)
          ...
          siglen =
              2 * (fp->ofdm_symbol_size + fp->nb_prefix_samples0) + (txsymb - 2) * (fp->ofdm_symbol_size + fp->nb_prefix_samples);
      } else {
...
static void fill_rf_config(RU_t *ru, char *rf_config_file)
{
  ...
  get_samplerate_and_bw(mu, N_RB, fp->threequarter_fs, &cfg->sample_rate, &cfg->samples_per_frame, &cfg->tx_bw, &cfg->rx_bw);
  ...
  if (cell_config->frame_duplex_type.value == 1 /* TDD */) {
    ...
    for (int slot = 0; slot < split7->n_tdd_period; ++slot) {
      for (int sym = 0; sym < 14; ++sym) {
        split7->slot_dirs[slot].sym_dir[sym] =
            tdd_table->max_tdd_periodicity_list[slot].max_num_of_symbol_per_slot_list[sym].slot_config.value;
      }
    }
  }
}
...
int setup_RU_buffers(RU_t *ru)
{
  ...
  if (config->cell_config.frame_duplex_type.value == TDD) {
    int N_TA_offset = config->carrier_config.uplink_frequency.value < 6000000
                          ? 400
                          : 431; // reference samples  for 25600Tc @ 30.72 Ms/s for FR1, same @ 61.44 Ms/s for FR2
    double factor = 1;
    ...
    LOG_I(PHY,
          "RU %d Setting N_TA_offset to %d samples (factor %f, UL Freq %d, N_RB %d, mu %d)\n",
          ru->idx,
          ru->N_TA_offset,
          factor,
          config->carrier_config.uplink_frequency.value,
          N_RB,
          mu);
  } else
    ru->N_TA_offset = 0;
...
void *ru_stats_thread(void *param)
{
  ...
  while (!oai_exit) {
    ...
    if (opp_enabled == 1) {
      if (ru->feprx)
        print_meas(&ru->ofdm_demod_stats, "feprx (all ports)", NULL, NULL);

      if (ru->feptx_ofdm) {
        print_meas(&ru->precoding_stats,
                   (ru->half_slot_parallelization == 0) ? "feptx_prec (per port)" : "feptx_prec (per port, half_slot)",
                   NULL,
                   NULL);
        print_meas(&ru->ofdm_mod_stats,
                   (ru->half_slot_parallelization == 0) ? "feptx_ofdm (per port)" : "feptx_ofdm (per port, half_slot)",
                   NULL,
                   NULL);
      ...
      if (ru->if_south == REMOTE_IF5)
        print_meas(&ru->ifdevice.tx_fhaul, "tx_fhaul (IF5)", NULL, NULL);
      else
    ...
    if (frame_tx == print_frame) {
      for (int i = 0; i < ru->nb_tx; i++) {
        ...
        if (slot_tx == 9) {
          ...
          if (output_fd) {
            fwrite(&ru->common.txdata[i][0], sizeof(int32_t), fp->samples_per_frame, output_fd);
            fclose(output_fd);
          } else {
...
void *ru_thread(void *param)
{
  ...
  while (!oai_exit) {
    ...
    LOG_D(PHY,
          ...
          proc->frame_tx,
          proc->tti_tx,
          RC.gNB[0]->proc.frame_rx,
          RC.gNB[0]->proc.slot_rx,
        ...
        if (prach_id >= 0) {
          ...
          T(T_GNB_PHY_PRACH_INPUT_SIGNAL,
            ...
            T_BUFFER(&ru->common.rxdata[0][fp->get_samples_slot_timestamp(proc->tti_rx - 1, fp, 0)] /*-ru->N_TA_offset*/,
                     fp->get_samples_per_slot(proc->tti_rx, fp) * 4 * 2));
...
void configure_rru(void *ruu, void *arg)
{
  ...
  // ru->nr_frame_parms->pdsch_config_common.referenceSignalPower                 =
  // ru->max_pdschReferenceSignalPower-config->att_tx[0];
...
void init_NR_RU(configmodule_interface_t *cfg, char *rf_config_file)
{
  ...
  for (ru_id = 0; ru_id < RC.nb_RU; ru_id++) {
    ...
    if (ru->num_gNB > 0) {
      LOG_D(PHY,
            "%s() RC.ru[%d].num_gNB:%d ru->gNB_list[0]:%p RC.gNB[0]:%p rf_config_file:%s\n",
            __FUNCTION__,
            ru_id,
            ru->num_gNB,
            ru->gNB_list[0],
